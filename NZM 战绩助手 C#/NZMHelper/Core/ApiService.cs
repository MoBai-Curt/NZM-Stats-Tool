...
namespace NZMHelper.Core
{
    public class ApiService
    {
        ...
        {
            try { File.WriteAllText("nzm_session.json", JsonConvert.SerializeObject(new { c, t = DateTimeOffset.Now.ToUnixTimeSeconds(), type = t })); } catch { }
        }
        ...
        {
            _cookie = null;
            if (File.Exists("nzm_session.json")) File.Delete("nzm_session.json");
        }
        ...
        public async Task<JToken> PostApiAsync(string method, object param)
        {
            ...
            try
            {
                ...
                {
                    req.Headers.Referrer = new Uri("https://nzm.qq.com/");
                }
