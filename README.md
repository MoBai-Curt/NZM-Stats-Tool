...
由于官方网页端严苛的安全风控（WAF）以及移动端底层的协议劫持，常规的 WebView 扫码登录已失效。本项目进行了底层重构，采用了 **“极简纯净的 Cookie 直连模式”** 及 **“原生无感代理抓包”**，彻底解决登录难题。
...
1. **获取凭证**：
   * **推荐方法**：在电脑上运行 `NZMHelper.exe`，成功抓取 Cookie 后，点击复制并发送到您的手机。
   * **备用方法**：在手机端使用 Alook 浏览器等抓包工具，登录官方活动页提取请求头中的 `Cookie`。
   * **极客方法**：使用仓库内附带的 `main.py` (MITM 抓包脚本) 在电脑上拦截获取。
